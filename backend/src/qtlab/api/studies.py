...
from qtlab.api.schemas import DefineResearchQuestionRequest, StudyResponse
...
def list_studies(session: Session = Depends(get_session),) -> list[StudyResponse]:
    repository = SQLiteStudyRepository(session)
...
@router.patch("/{study_id}/research-question", response_model=StudyResponse, )
def define_research_question(study_id: UUID,
                             request: DefineResearchQuestionRequest,
                             session: Session = Depends(get_session),) -> StudyResponse:
    repository = SQLiteStudyRepository(session)
    study = repository.get(study_id)
    ...
    study.define_research_question(research_question)
